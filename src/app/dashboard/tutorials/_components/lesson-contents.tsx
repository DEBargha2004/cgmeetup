...
export default function LessonContents({
  ...
  return (
    <>
      ...
      <div className="p-2 w-full flex justify-center items-center @container">
        <div
          className={cn(
            "@lg:w-4/5 w-full shrink-0",
            "grid @md:grid-cols-[auto_1fr] gap-x-4 gap-y-2"
          )}
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="h-full">
                <FormControl>
                  <Textarea {...field} className="w-full h-full" />
                </FormControl>
              </FormItem>
            )}
          />

          <p className="p-0.5 px-1 bg-black/60 rounded w-fit @md:col-start-2">
            <span className="opacity-80">{formatTimeFromMinutes(23)}</span>
          </p>
        </div>
      </div>
      <div className="p-2">
